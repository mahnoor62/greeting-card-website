...
const LeaderBoard = () => {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(LeaderBoard, 5000);
  ...
  return (
    <>
      <Head>
        <title>LeaderBoard of {gameSlug} || {process.env.NEXT_PUBLIC_APP_NAME}</title>
      </Head>
      <Box
        sx={{
        ...
        >
          <Box sx={{ display: 'flex', width: '100%' }}>
            <Box sx={{ width: '100%', textAlign: 'center' }}>
              <Typography variant="h4" sx={{ marginBottom: '20px' }}>Leaderboard</Typography>
            </Box>
          </Box>
          <div>
